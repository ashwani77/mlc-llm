...
# Generate build configuration interactively
# Configured for CPU-only build (works on all platforms)
...
# Using limited parallelism to avoid memory issues
RUN make -j$(( $(nproc) < 4 ? $(nproc) : 4 )) && \
    echo "✓ MLC-LLM libraries built successfully"
